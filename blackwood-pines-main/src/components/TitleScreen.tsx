        ...
        <div className="w-full max-w-4xl bg-zinc-950/90 border border-zinc-800/90 rounded-2xl p-6 sm:p-8 shadow-2xl backdrop-blur-md min-h-[400px] flex flex-col justify-between">
          ...
          {activeTab === 'play' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="pt-4 flex flex-col gap-3 border-t border-zinc-800/80">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    {hasAutoSave && onContinueAutoSave && (
                      <button
                        id="resume-autosave-btn"
                        onClick={() => {
                          horrorAudio.playMenuSelect();
                          onContinueAutoSave();
                        }}
                        onMouseEnter={handleHover}
                        className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-6 py-4 bg-sky-800 hover:bg-sky-700 active:bg-sky-900 text-white font-bold text-xs tracking-widest uppercase rounded-xl shadow-[0_0_20px_rgba(14,165,233,0.35)] transition-all cursor-pointer"
                      >
                        <RotateCcw className="h-4 w-4 text-sky-200" />
                        <span>RESUME LAST SAVE</span>
                      </button>
                    )}
                    <button
                      id="start-game-btn"
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  {onLoadSaveClick && (
                    <button
                      id="title-load-code-btn"
                      onClick={() => {
                        horrorAudio.playMenuSelect();
                        onLoadSaveClick();
                      }}
                      onMouseEnter={handleHover}
                      className="inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-700 text-zinc-300 hover:text-white rounded-lg text-xs font-semibold transition-colors cursor-pointer"
                    >
                      <Download className="h-3.5 w-3.5 text-sky-400" />
                      <span>ENTER SAVE CODE</span>
                    </button>
                  )}
                  {onOpenObjectives && (
                    <button
                      id="title-objectives-btn"
                      onClick={() => {
                        horrorAudio.playMenuSelect();
                        onOpenObjectives();
                      }}
                    ...
                    >
                      ...
                      <span>VIEW 100+ OBJECTIVES DOSSIER</span>
                    </button>
                  )}
                </div>
              </div>
