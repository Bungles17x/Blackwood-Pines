...
export class HorrorEngine {
  ...
  // Tracking Stats for Objectives & Save Game
  public distanceTraveledMeters: number = 0;
  public chasesEscaped: number = 0;
  public bottlesThrown: number = 0;
  public flaresUsed: number = 0;
  public lockersUsed: number = 0;
  public batteriesUsed: number = 0;
  private virtualStickX: number = 0;
  private virtualStickY: number = 0;
  ...
  public setVirtualJoystick(x: number, y: number) {
    if (!this.isRunning || this.isModalOpen || this.isDying) return;
    if (this.isPlayerHiding && (Math.abs(x) > 0.1 || Math.abs(y) > 0.1)) this.exitLocker();
    this.virtualStickX = Math.max(-1, Math.min(1, x));
    this.virtualStickY = Math.max(-1, Math.min(1, y));
  }

  public applyLookDelta(deltaX: number, deltaY: number) {
    if (this.isModalOpen || !this.isRunning || this.isDying) return;
  ...
  private update(delta: number) {
    ...
    if (!this.isPlayerHiding) {
      ...
      if (Math.abs(this.virtualStickY) > 0.01) {
        wishDir.addScaledVector(forward, this.virtualStickY);
      }
    ...
    if (isMoving) {
      ...
      // Footstep audio & mobile haptic feedback
      if (this.isGrounded) {
        ...
        if (this.footstepTimer > stepInterval) {
          ...
          horrorAudio.triggerHaptic(this.isSprinting ? 18 : 8);
    ...
    if (!this.isPlayerHiding) {
      ...
      // Distance & POI exploration tracking
      const frameDist = Math.hypot(this.playerVelocity.x * delta, this.playerVelocity.z * delta);
      if (frameDist > 0.001) {
        this.distanceTraveledMeters += frameDist;
        const totalDistInt = Math.floor(this.distanceTraveledMeters);
        objectivesManager.setProgress('explore_dist_100', totalDistInt);
        objectivesManager.setProgress('explore_dist_250', totalDistInt);
        objectivesManager.setProgress('explore_dist_500', totalDistInt);
        objectivesManager.setProgress('explore_dist_1000', totalDistInt);
        objectivesManager.setProgress('explore_dist_2000', totalDistInt);
        objectivesManager.setProgress('explore_dist_3500', totalDistInt);
        if (this.isCrouching) {
          objectivesManager.addProgress('stealth_crouch_walk', frameDist);
          objectivesManager.addProgress('stealth_crouch_150', frameDist);
        }
      }

      const curX = this.playerPosition.x;
      const curZ = this.playerPosition.z;
      if (Math.hypot(curX - (-1), curZ - (-20)) < 7.5) {
        objectivesManager.complete('explore_truck_wreck');
      }
      if (Math.hypot(curX - (-28), curZ - 12) < 7.5) {
        objectivesManager.complete('explore_stone_circle');
      }
      if (Math.hypot(curX - (-15), curZ - 8) < 7.5) {
        objectivesManager.complete('explore_logging_camp');
      }
      if (Math.hypot(curX - 18, curZ - 14) < 7.5) {
        objectivesManager.complete('explore_generator_shed');
      }
      if (curZ > 42) {
        objectivesManager.complete('explore_south_gate');
      }
      if (curX >= 12 && curX <= 34 && curZ >= -34 && curZ <= -8) {
        objectivesManager.complete('story_cabin_enter');
        if (Math.hypot(curX - 22, curZ - (-18)) < 4.5) {
          objectivesManager.complete('explore_old_chimney');
        }
      }
    ...
    if (this.sensorNotifyTimer <= 0) {
      ...
      // Top Compass Heading & Waypoint computation
      const headingDeg = Math.round(((-this.yaw * 180 / Math.PI) % 360 + 360) % 360);
      let targetPos = new THREE.Vector3(-1, 0, -20.8);
      let targetLabel = 'TRUCK HOOD';

      if (!this.inventory.hasKeycard && !this.isSubstationUnlocked) {
        targetPos = new THREE.Vector3(-1, 0, -20.8);
        targetLabel = 'TRUCK (KEY)';
      } else if (!this.isSubstationUnlocked) {
        targetPos = new THREE.Vector3(16, 0, -20);
        targetLabel = 'RANGER CABIN';
      } else if (this.isPowerRestored) {
        targetPos = new THREE.Vector3(0, 0, 48.5);
        targetLabel = 'HIGHWAY EXIT';
      } else if (this.inventory.fuses < 3 || !this.inventory.hasFuelCan) {
        if (!this.inventory.hasFuelCan) {
          targetPos = new THREE.Vector3(-15, 0, 8);
          targetLabel = 'DIESEL FUEL';
        } else {
          targetPos = new THREE.Vector3(-28, 0, 12);
          targetLabel = 'SPARK PLUGS';
        }
      } else {
        targetPos = new THREE.Vector3(18, 0, 14);
        targetLabel = 'POWER SHED';
      }

      const dx = targetPos.x - this.playerPosition.x;
      const dz = targetPos.z - this.playerPosition.z;
      const distanceMeters = Math.max(1, Math.round(Math.hypot(dx, dz)));
      const targetAngleRad = Math.atan2(dx, -dz);
      const targetBearingDeg = (targetAngleRad * 180 / Math.PI + 360) % 360;
      const relBearingDeg = ((targetBearingDeg - headingDeg + 540) % 360) - 180;

      this.onCompassChange?.(headingDeg, {
        label: targetLabel,
        distanceMeters,
        relBearingDeg,
      });
  ...
  public getSaveData(chapterId?: number, timeSurvivedSeconds?: number): SaveData {
    const activeChapter = chapterId ?? this.currentChapterId ?? 1;
    const activeTime = timeSurvivedSeconds ?? Math.floor(this.timeSurvivedSeconds ?? 0);
    return {
      version: 1,
      timestamp: Date.now(),
      chapterId: activeChapter,
      timeSurvivedSeconds: activeTime,
      ...
      player: {
        ...
        isPlayerHiding: this.isPlayerHiding,
        bodyTemp: this.survivalVitals.bodyTemp,
      },
      inventory: { ...this.inventory },
      flashlight: { ...this.flashlightState },
      world: {
        isPowerRestored: this.isPowerRestored,
        isSubstationUnlocked: this.isSubstationUnlocked,
        isPickupHoodOpen: this.isPickupHoodOpen,
        collectedItemIds: this.items.filter((it) => it.collected).map((it) => it.id),
        notesRead: [...this.inventory.notesRead],
        recoilPulls: this.survivalVitals.recoilPulls,
      },
      creature: {
        x: this.creature ? this.creature.position.x : 0,
        y: this.creature ? this.creature.position.y : 0,
        z: this.creature ? this.creature.position.z : 0,
        isAwakened: this.isMonsterAwakened,
      },
      stats: {
        distanceTraveledMeters: this.distanceTraveledMeters,
        chasesEscaped: this.chasesEscaped,
        bottlesThrown: this.bottlesThrown,
        flaresUsed: this.flaresUsed,
        lockersUsed: this.lockersUsed,
        batteriesUsed: this.batteriesUsed,
      },
      completedObjectiveIds: objectivesManager.getCompletedIds(),
    };
  }

  public loadSaveData(save: SaveData) {
    this.isModalOpen = false;
    this.wasRunningBeforeModal = false;
    this.currentChapterId = save.chapterId || 1;
    this.timeSurvivedSeconds = save.timeSurvivedSeconds || 0;
    this.clearInputState();

    this.playerPosition.set(save.player.x, save.player.y, save.player.z);
    this.camera.position.copy(this.playerPosition);
    ...
    // Orient flashlight directly with camera immediately
    const rightDir = new THREE.Vector3(1, 0, 0).applyQuaternion(this.camera.quaternion);
    const upDir = new THREE.Vector3(0, 1, 0).applyQuaternion(this.camera.quaternion);
    const handOffset = rightDir.clone().multiplyScalar(0.18).add(upDir.clone().multiplyScalar(-0.16));
    this.flashlight.position.copy(this.camera.position).add(handOffset);
    if (this.flashlightSpill) {
      this.flashlightSpill.position.copy(this.flashlight.position);
    }
    const forwardDir = new THREE.Vector3(0, 0, -1).applyQuaternion(this.camera.quaternion);
    this.flashlightTarget.position.copy(this.camera.position).add(forwardDir.multiplyScalar(30));

    this.stamina = save.player.stamina;
    this.isCrouching = save.player.isCrouching;
    this.isPlayerHiding = save.player.isPlayerHiding;
    if (save.player.bodyTemp !== undefined) {
      this.survivalVitals.bodyTemp = save.player.bodyTemp;
    }

    this.inventory = { ...save.inventory };
    this.flashlightState = { ...save.flashlight };
    this.flashlight.visible = this.flashlightState.enabled;

    this.isPowerRestored = save.world.isPowerRestored;
    this.isSubstationUnlocked = save.world.isSubstationUnlocked;
    this.isPickupHoodOpen = save.world.isPickupHoodOpen;

    if (this.pickupHoodHinge) {
      this.pickupHoodHinge.rotation.x = this.isPickupHoodOpen ? -Math.PI * 0.42 : 0;
    }
    if (this.substationDoorMesh) {
      this.substationDoorMesh.rotation.y = this.isSubstationUnlocked ? Math.PI / 2.2 : 0;
    }
    if (this.exitDoorMesh) {
      this.exitDoorMesh.position.set(0, this.isPowerRestored ? 5.7 : 1.9, 48.5);
    }
    if (save.world.recoilPulls !== undefined) {
      this.survivalVitals.recoilPulls = save.world.recoilPulls;
    }

    // Re-populate collected items and refresh scene meshes
    this.itemMeshes.forEach((mesh) => this.scene.remove(mesh));
    this.itemMeshes.clear();
    const { items } = getFacilityLayout();
    this.items = items;
    const collectedSet = new Set(save.world.collectedItemIds || []);
    this.items.forEach((it) => {
      if (collectedSet.has(it.id)) {
        it.collected = true;
      }
    });
    this.updateActiveWalls();
    ...
    // Creature state
    if (save.creature && this.creature) {
      this.creature.position.set(save.creature.x, save.creature.y, save.creature.z);
      this.isMonsterAwakened = save.creature.isAwakened;
    }

    // Stats
    if (save.stats) {
      this.distanceTraveledMeters = save.stats.distanceTraveledMeters || 0;
      this.chasesEscaped = save.stats.chasesEscaped || 0;
      this.bottlesThrown = save.stats.bottlesThrown || 0;
      this.flaresUsed = save.stats.flaresUsed || 0;
      this.lockersUsed = save.stats.lockersUsed || 0;
      this.batteriesUsed = save.stats.batteriesUsed || 0;
    }

    // Objectives sync
    if (save.completedObjectiveIds) {
      objectivesManager.reset(save.completedObjectiveIds);
    }

    this.onInventoryChange?.({ ...this.inventory });
    this.onFlashlightChange?.({ ...this.flashlightState });
    this.onPromptChange?.(null);
    this.onHidingChange?.(this.isPlayerHiding);
    this.onCrouchChange?.(this.isCrouching);
    this.onStaminaChange?.(this.stamina);
    this.onVitalsChange?.({ ...this.survivalVitals });

    const msg = `EXPEDITION RESUMED // CHAPTER ${save.chapterId}`;
    this.bannerMessage = msg;
    this.onBannerMessage?.(msg);
