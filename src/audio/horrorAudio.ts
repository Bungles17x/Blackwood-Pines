...
class HorrorAudioEngine {
  ...
  public playObjectivePing() {
    try {
      ...
      osc2.frequency.exponentialRampToValueAtTime(1174.66, t + 0.2); // D6
  ...
  public playThunder() {
    try {
      ...
      for (let i = 0; i < bufferSize; i++) {
        ...
        const env = Math.sin((i / bufferSize) * Math.PI) * Math.exp(-i / (bufferSize * 0.6));
        data[i] = last * env * 3.5;
      }
      ...
      lowpass.frequency.linearRampToValueAtTime(45, t + duration);
  ...
  /**
   * Device haptic vibration feedback for mobile browsers
   */
